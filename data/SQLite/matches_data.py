...
def import_atp_data_to_sqlite(db_path="data/SQLite/tennis.db", table_name="atp_matches"):
    ...
    for k in [3, 5, 10, 25, 50, 100]:
        ...
        wins_last_k = []
        for w_id, l_id in tqdm(zip(all_data_filtered['winner_id'], all_data_filtered['loser_id']),
                                 total=len(all_data_filtered)):
        ...
        col_name = "WIN_LAST_" + str(k) + "_DIFF"
        final_data[col_name] = wins_last_k
    ...
    for k in [3, 5, 10, 20, 50, 100, 200, 300, 2000]:
        last_k_matches_perf = defaultdict(lambda: defaultdict(lambda: deque(maxlen=k)))
        p_ace_k = []
        p_df_k = []
        p_1stIn_k = []
        p_1stWon_k = []
        ...
        for row in tqdm(all_data_filtered.itertuples(index=False), total=len(all_data_filtered)):
            ...
            p_ace_k.append(mean(last_k_matches_perf[w_id]["p_ace"]) - mean(last_k_matches_perf[l_id]["p_ace"]))
            p_df_k.append(mean(last_k_matches_perf[w_id]["p_df"]) - mean(last_k_matches_perf[l_id]["p_df"]))
            p_1stIn_k.append(mean(last_k_matches_perf[w_id]["p_1stIn"]) - mean(last_k_matches_perf[l_id]["p_1stIn"]))
            p_1stWon_k.append(mean(last_k_matches_perf[w_id]["p_1stWon"]) - mean(last_k_matches_perf[l_id]["p_1stWon"]))
            p_2ndWon_k.append(mean(last_k_matches_perf[w_id]["p_2ndWon"]) - mean(last_k_matches_perf[l_id]["p_2ndWon"]))
            p_bpSaved_k.append(mean(last_k_matches_perf[w_id]["p_bpSaved"]) - mean(last_k_matches_perf[l_id]["p_bpSaved"]))

            if (w_svpt != 0) and (w_svpt != w_1stIn):
                last_k_matches_perf[w_id]["p_ace"].append(100 * (w_ace / w_svpt))
                last_k_matches_perf[w_id]["p_df"].append(100 * (w_df / w_svpt))
                last_k_matches_perf[w_id]["p_1stIn"].append(100 * (w_1stIn / w_svpt))
                last_k_matches_perf[w_id]["p_2ndWon"].append(100 * (w_2ndWon / (w_svpt - w_1stIn)))
            if l_svpt != 0 and (l_svpt != l_1stIn):
                last_k_matches_perf[l_id]["p_ace"].append(100 * (l_ace / l_svpt))
                last_k_matches_perf[l_id]["p_df"].append(100 * (l_df / l_svpt))
                last_k_matches_perf[l_id]["p_1stIn"].append(100 * (l_1stIn / l_svpt))
                last_k_matches_perf[l_id]["p_2ndWon"].append(100 * (l_2ndWon / (l_svpt - l_1stIn)))
            if w_1stIn != 0:
                last_k_matches_perf[w_id]["p_1stWon"].append(100 * (w_1stWon / w_1stIn))
            if l_1stIn != 0:
                last_k_matches_perf[l_id]["p_1stWon"].append(100 * (l_1stWon / l_1stIn))
            if w_bpFaced != 0:
                last_k_matches_perf[w_id]["p_bpSaved"].append(100 * (w_bpSaved / w_bpFaced))
            if l_bpFaced != 0:
                last_k_matches_perf[l_id]["p_bpSaved"].append(100 * (l_bpSaved / l_bpFaced))
    ...
    #     recording individual overall ELO ratings and the ELO ratings on the match's surface.
    # Overall ELO:
    elo_players = defaultdict(int)
    all_elo = defaultdict(lambda: deque())
    df_elo = []
    winner_elo = []  # Overall winner ELO
    loser_elo = []   # Overall loser ELO
    for w_id, l_id in tqdm(zip(all_data_filtered['winner_id'], all_data_filtered['loser_id']),
                             total=len(all_data_filtered)):
        k_val = 24
        ...
        winner_elo.append(elo_w)
        loser_elo.append(elo_l)
        elo_players[w_id] = elo_w
        elo_players[l_id] = elo_l
        all_elo[w_id].append(elo_w)
        all_elo[l_id].append(elo_l)
    final_data["ELO_DIFF"] = df_elo
    final_data["WINNER_ELO"] = winner_elo
    final_data["LOSER_ELO"] = loser_elo

    # ELO per surface:
    elo_surfaces = defaultdict(lambda: defaultdict(int))
    all_elo_surfaces = defaultdict(lambda: defaultdict(lambda: deque()))
    df_elo_surface = []
    winner_elo_surface = []  # Winner's ELO on the match's surface
    loser_elo_surface = []   # Loser's ELO on the match's surface
    for w_id, l_id, surface in tqdm(zip(all_data_filtered['winner_id'], 
                                        all_data_filtered['loser_id'], 
                                        all_data_filtered['surface']),
                                     total=len(all_data_filtered)):
        k_val = 24
        ...
        elo_w += k_val * (1 - exp_w)
        elo_l += k_val * (0 - exp_l)
        df_elo_surface.append(elo_w - elo_l)
        # Record the individual surface ELO ratings for this match
        winner_elo_surface.append(elo_w)
        loser_elo_surface.append(elo_l)
    ...
    final_data["ELO_SURFACE_DIFF"] = df_elo_surface
    final_data["WINNER_ELO_SURFACE"] = winner_elo_surface
    final_data["LOSER_ELO_SURFACE"] = loser_elo_surface

    # 9) Calculate the gradient difference on total ELO over various window sizes
